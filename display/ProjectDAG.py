...
def regression():
    ...
    x = pd.get_dummies(x)
    y1d = [y.iloc[i][0] for i in range(len(y))]
    ...
    RR = sklearn.linear_model.Ridge(alpha=0.1)
    RR.fit(x, y)
    pickle.dump(RR, open(pathRidgeRegression1, 'wb'))

    GB = GradientBoostingRegressor()
    GB.fit(x, y1d)
    pickle.dump(RR, open(pathGradientBoosting, 'wb'))

    SVR = SVR(kernel="rbf", C=100, gamma=0.1, epsilon=0.1)
    SVR.fit(x, y1d)
    pickle.dump(LR, open(pathSVR, 'wb'))

    RF = RandomForestRegressor(n_estimators = 1000)
    RF.fit(x, y1d)
    pickle.dump(RF, open(pathRandomForest, 'wb'))

    AB = AdaBoostRegressor(n_estimators=100)
    AB.fit(x, y1d)
    pickle.dump(AB, open(pathAdaBoost, 'wb'))
